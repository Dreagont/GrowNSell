...
public class Database : ScriptableObject
{
    ...
    public void SetItemID()
    {
        ...
        var hadIDInRange = foundItems.Where(i => i.ID != -1 && i.ID < foundItems.Count).OrderBy(i => i.ID).ToList();
        var hadIDNotInRange = foundItems.Where(i => i.ID != -1 && i.ID >= foundItems.Count).OrderBy(i => i.ID).ToList();
        ...
        for (int i = 0; i < foundItems.Count; i++)
        {
            ...
            itemToAdd = hadIDInRange.Find(d => d.ID == i);
        ...
        foreach (var item in hadIDNotInRange)
        {
